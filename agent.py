...
""".strip()
    ...
    try:
        ...
        if not isinstance(
            ...
            raise ValueError(
                "LLM response did not contain a valid parameters object"
            )
...
The previous circuit was evaluated by a deterministic geometry
engine running inside a Daytona sandbox.

Produce a NEW parameter set that addresses the weaknesses shown
by the evaluation.
...
6. Why moving away from the previous parameter values is justified.
7. How the proposed changes should address the score.
8. Whether the layout style itself should change.
...
""".strip()
    ...
    try:
        ...
        decision = AgentDecision(
            ...
            parameter_reasoning=(
                "Parameters were changed within their "
                "allowed ranges as a fallback."
            ),
            direction_reasoning=(
                "The existing direction was retained "
                "because the fallback mutation does not "
                "change direction."
            ),
            previous_parameters=previous,
            new_parameters=asdict(
...
def fallback_parameters(
    ...
    rng = random.Random(
        100000 + circuit_id
    )  # NOSONAR - non-cryptographic geometry generation
...
def mutate_parameters(
    ...
    rng = random.Random(
        time.time_ns()
    )  # NOSONAR - non-cryptographic optimisation mutation
...
def generate_circuit_points(
    parameters: CircuitParameters,
    seed: int,
) -> list[tuple[float, float]]:  # noqa: S3776

    rng = random.Random(
        seed
    )  # NOSONAR - deterministic non-cryptographic geometry generation
...
SANDBOX_FUNCTIONS = [
    ...
    calculate_complexity_score,
    calculate_variety_score,
    calculate_closure_score,
    score_circuit,
]


def build_sandbox_engine_source() -> str:
    source_parts = [
        "from __future__ import annotations",
        "",
        "import json",
        ...
        "",
        "# Constants required by the geometry and scoring functions.",
        "EPS = 1e-9",
        f"POINTS_PER_CORNER = {POINTS_PER_CORNER}",
        f"MAX_CURVATURE = {MAX_CURVATURE!r}",
        "",
        textwrap.dedent(
            inspect_source(
                CircuitScore
    ...
    source_parts.append(
        """
def run_evaluation(parameters, seed):
    parameter_object = SimpleNamespace(
        **parameters
    )
    ...
    return {
        ...
        "score": {
            ...
            "closure": float(score.closure),
            "separation": float(score.separation),
            "smoothness": float(score.smoothness),
            "complexity": float(score.complexity),
            "scale": float(score.scale),
            "variety": float(score.variety),
...
def create_daytona_sandbox() -> Any:
    sandbox = daytona.create()

    source = build_sandbox_engine_source()

    bootstrap_code = f"""
from pathlib import Path

engine_source = {source!r}

Path("/tmp/circuit_engine.py").write_text(
    engine_source,
    encoding="utf-8",
)

print("DAYTONA_ENGINE_READY")
"""
    ...
    if (
        "DAYTONA_ENGINE_READY"
        not in output
    ):

        raise RuntimeError(
            (
                "Daytona engine bootstrap failed. "
                f"Output: {output}"
            )
        )
...
import circuit_engine

parameters = json.loads(
    {parameter_json!r}
)

result = circuit_engine.run_evaluation(
    parameters,
    {seed},
)
...
"""
    ...
    if json_line is None:
        ...
        raise RuntimeError(
            (
                "Could not parse Daytona "
                f"evaluation output: {output}"
            )
        )
    ...
    score = CircuitScore(
        ...
        separation=float(
            score_data["separation"]
        ),
        smoothness=float(
            score_data["smoothness"]
        ),
        complexity=float(
        ...
        try:
            ...
            self.terminal_output.emit(
                (
                    ...
                    "Geometry, validation and scoring "
                    "will execute inside it."
                )
            )
            ...
            for iteration in range(
                ...
                self.terminal_output.emit(
                    (
                        f"[Circuit {circuit_id:03d}] "
                        f"Iteration {iteration:02d}: "
                        "executing generator and evaluator "
                        "inside Daytona..."
                    )
                )

                points, score, sandbox_output = (
                    evaluate_in_daytona(
                        sandbox,
                        parameters,
                        seed,
            ...
            if sandbox is not None:
                ...
                try:
                    ...
                    self.terminal_output.emit(
                        (
                            f"[Circuit {circuit_id:03d}] "
                            "WARNING: Daytona sandbox deletion "
...
class MainWindow(QMainWindow):
    ...
    def on_circuit_finished(
        ...
        if (
            worker is not None
            and worker.best_score > 0.0
        ):

            self.best_label.setText(
                (
                    "Global best score: "
                    f"{worker.best_score:.2f}"
                )
            )
...
def main() -> None:
    ...
    print(
        "Geometry and evaluation: Daytona sandbox"
    )

    print(
        "AI parameter selection: Groq"
    )
